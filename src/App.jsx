...
async function fetchRealPrice(ticker) {
  ...
  const data = await callClaude({
    ...
    max_tokens: 150, // OPT: reducido de 300 a 150 — solo necesitamos el JSON
  ...
  try {
    ...
    if (parsed.price > 0) {
      priceCache[ticker] = { data: parsed, ts: Date.now() }; // guardar en caché
      return parsed;
    }
  } catch {}
  return null;
...
async function fetchAnalysis(ticker, horizon, profile, priceInfo, months) {
  ...
  // OPT: System prompt dividido en parte estática (cacheable) + parte dinámica
  // La parte estática se cachea automáticamente por anthropic.js → ahorra ~80% tokens
  const systemStatic = `Eres un sistema multiagente de análisis de inversiones de alta precisión.
Analiza el activo financiero solicitado con estos 4 agentes: fundamental, tecnico, riesgo, sintesis.
REGLAS: Responde SOLO con JSON válido sin markdown. Cada agente mínimo 150 palabras.
Formato exacto:
{"fundamental":"...","tecnico":"...","riesgo":"...","sintesis":"...","veredicto":"COMPRAR|MANTENER|VENDER","confianza":70,"entrada":0,"stopLoss":0,"objetivo1":0,"objetivo2":0,"rr":2.0,"chart":[{"mes":"","precio":0,"ma20":0,"ma50":0}]}`;

  const systemDynamic = `Fecha: ${dateStr}. Precio actual: $${p||0} USD (fuente: ${priceInfo?.source||"estimado"}).
Meses del chart (exactamente estos 12): ${months.join(", ")}.
Último mes (${months[11]}) precio = $${p||0}.
Calcula: stopLoss=${p?(p*0.91).toFixed(2):0}, objetivo1=${p?(p*1.18).toFixed(2):0}, objetivo2=${p?(p*1.38).toFixed(2):0}.`;
  ...
  const raw = await callClaude({
    ...
    messages: [{
      role: "user",
      content: `Analiza ${ticker.toUpperCase()}. Horizonte: ${horizon}. Perfil: ${profile}. Precio: $${p} USD. Solo JSON.`
    }]
  });
