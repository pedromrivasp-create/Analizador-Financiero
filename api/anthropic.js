// api/anthropic.js — proxy seguro con rate limiting por IP + bypass para suscriptores Pro
const UPSTASH_URL   = process.env.UPSTASH_REDIS_REST_URL;
const UPSTASH_TOKEN = process.env.UPSTASH_REDIS_REST_TOKEN;
const DAILY_LIMIT = 10; // análisis por IP por día (solo aplica a no-Pro)
...
// Consulta si este correo está marcado como Pro en Upstash.
// Si Upstash falla, devolvemos false (no privilegiamos por error) — el usuario
// simplemente cae al límite normal de 10/día en vez de quedar bloqueado del todo.
async function isProEmail(email) {
  if (!email) return false;
  try {
    const key = `subscription:${String(email).toLowerCase().trim()}`;
...
export default async function handler(req, res) {
  ...
  if (isAnalysis) {
    ...
    if (pro) {
      ...
      console.log(`[anthropic] Pro confirmado (${email}) — sin límite`);
    } else {
